...
def test_apartments_com_run_persists_observations(tmp_path: Path) -> None:
    catchment = Catchment("48097", ("76240",), (33.5, -97.4, 33.8, -96.8), None)
    htmls = [(FIXTURES / "apartments_com_search.html").read_text(encoding="utf-8")]
    config = {
        "sources": {
            "apartments_com": {"enabled": True, "_test_htmls": htmls},
        },
        "paths": {
    ...
    apply_schema(conn)
    n = conn.execute(
        "SELECT COUNT(*) FROM raw_observations WHERE source='apartments_com'"
    ).fetchone()[0]
    # Tower View (1-2 bed range) -> 1 obs (no fan-out); Oak Villa (2 bed) -> 1 obs. Total = 2.
    assert n == 2
